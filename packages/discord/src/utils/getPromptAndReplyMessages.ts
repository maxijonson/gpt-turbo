...
const PROMPT_NOT_FOUND_MESSAGE =
 ...
 * @throws {BotException} If the user's reply is empty. (Usually happens if the user doesn't enable pinging when replying.)
 * @throws {BotException} If the user's reply is to an error message.
 * @throws {BotException} If the prompt message is not found.
 */
export default async (
    ...
    if (!promptMessage) {
        throw new BotException(PROMPT_NOT_FOUND_MESSAGE);
    }
